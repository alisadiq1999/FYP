...
from helper_functions.LibandMethods import *

# Wrting in file
csvfile=open('OCD_Report.csv','w', newline='')#opening file
...
emotion_model_path = 'models/fer2013.hdf5' #storing expression model path in var
emotion_labels = get_labels()#loading labels
predictor = dlib.shape_predictor("models/shape_predictor_68_face_landmarks.dat")# init dlib's face detector (HOG-based)
...
while True:
    ...
    for (x, y) in temp:
    	a = a + 1
    	if a== 9 or a == 31 or a == 37 or a == 46 or a == 49 or a == 55 or a==63 or a==67: #chin  - Nose - left Eye - Right Eye  - Left mouth - right Mouth
...
#----------------------------------------------------------------------------------------------
    ...
    text_talk = detect_talking(rgb_image, landmarks)
    if len(list_for_talking) <= 24:
        list_for_talking.append(text_talk)
    elif len(list_for_talking) > 24:
        list_for_talking.remove(list_for_talking[0])
        list_for_talking.append(text_talk)

    put_text = "Not Talking"

    if list_for_talking.count("Talking") > 8:
        put_text = "Talking"
        talk_col = (255, 0, 0)
    ...
    cv2.putText(rgb_image, put_text, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, talk_col,
    ...
    cv2.imshow('OCD', bgr_image)#Show Final Output
